...
def get_attack_type_from_folder(folder_path):
    """Extract attack type from folder path"""
    folder_name = folder_path.split('/')[-1]
    if 'WebDDoS' in folder_name or '00-WebDDoS' in folder_name:
        return 'WebDDoS'
    elif 'LDAP' in folder_name or '01-LDAP' in folder_name:
        return 'LDAP'
    elif 'Portmap' in folder_name or '02-Portmap' in folder_name:
        return 'Portmap'
    elif 'DNS' in folder_name or '03-DNS' in folder_name:
        return 'DNS'
    elif 'UDPLag' in folder_name or '04-UDPLag' in folder_name:
        return 'UDPLag'
    elif 'NTP' in folder_name or '05-NTP' in folder_name:
        return 'NTP'
    elif 'SNMP' in folder_name or '06-SNMP' in folder_name:
        return 'SNMP'
    elif 'SSDP' in folder_name or '07-SSDP' in folder_name:
        return 'SSDP'
    elif 'Syn' in folder_name or '08-Syn' in folder_name:
        return 'Syn'
    elif 'TFTP' in folder_name or '09-TFTP' in folder_name:
        return 'TFTP'
    elif 'UDP' in folder_name or '10-UDP' in folder_name:
        return 'UDP'
    elif 'NetBIOS' in folder_name or '11-NetBIOS' in folder_name:
        return 'NetBIOS'
    elif 'MSSQL' in folder_name or '12-MSSQL' in folder_name:
        return 'MSSQL'
    else:
        return 'benign'
...
def parse_labels(dataset_type=None, folder_path=None, attackers=None, victims=None, label=1):
    output_dict = {}

    # Determine attack type from folder path if available
    if folder_path is not None:
        attack_type = get_attack_type_from_folder(folder_path)
        label = ATTACK_CLASSES[attack_type]
...
def count_flows(preprocessed_flows):
    ...
    for flow in preprocessed_flows:
        ...
        if label not in class_flows:
            class_flows[label] = 0
            class_fragments[label] = 0
        
        class_flows[label] += 1
        class_fragments[label] += flow_fragments
    
    benign_flows = class_flows.get(0, 0)
    attack_flows = sum(count for label, count in class_flows.items() if label != 0)
    benign_fragments = class_fragments.get(0, 0)
    attack_fragments = sum(count for label, count in class_fragments.items() if label != 0)
    
    return (total_flows, attack_flows, benign_flows), (total_fragments, attack_fragments, benign_fragments)
...
def balance_dataset(flows, total_fragments=float('inf')):
    ...
    # Count fragments by class
    class_fragments = {}
    for flow in flows:
        label = flow[1]['label']
        fragments = len(flow[1]) - 1
        if label not in class_fragments:
            class_fragments[label] = 0
        class_fragments[label] += fragments
    
    if len(class_fragments) == 0:
        return flows, 0, 0
    
    # Find minimum fragments per class (for balancing)
    min_fragments = min(total_fragments / len(class_fragments), min(class_fragments.values()))
    
    random.shuffle(flows)
    new_class_fragments = {label: 0 for label in class_fragments.keys()}
    
    for flow in flows:
        label = flow[1]['label']
        flow_fragments = len(flow[1]) - 1
        
        if new_class_fragments[label] + flow_fragments <= min_fragments:
            new_class_fragments[label] += flow_fragments
            new_flow_list.append(flow)
    
    total_benign = new_class_fragments.get(0, 0)
    total_attack = sum(count for label, count in new_class_fragments.items() if label != 0)
    
    return new_flow_list, total_benign, total_attack
